...
u32 LoadGame( void )
{
	...
	*(vu32*)0x800000FC = 729000000;				// CPU Clock Speed		
	...
	{
		PrintFormat( MENU_POS_X, MENU_POS_Y + 20*10, "Fatal error app_entry is not within the apploader area!");
		sleep(10);
		exit(0);
	}

	PrintFormat( MENU_POS_X, MENU_POS_Y + 20*12, "Apploader OK\n");
	...
	PrintFormat( MENU_POS_X, MENU_POS_Y + 20*13, "Loading main.dol\n");
	...
	while (1)
	{
		...
		PrintFormat( MENU_POS_X + counter * 8, MENU_POS_Y + 20*13, ".");
...
#endif
			PrintFormat( MENU_POS_X, MENU_POS_Y + 20*10, "Fatal error app_main:Data not in MEM1!");
			sleep(10);
			exit(0);
		}
#ifdef DEBUG
	...
	{
		PrintFormat( MENU_POS_X, MENU_POS_Y + 20*10, "Fatal error entrypoint is not within MEM1!");
		sleep(10);
		exit(0);
	}
