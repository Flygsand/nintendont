...
void DIinit( void )
{
	...
	if( ret != FR_OK )
	{
		sprintf( GamePath, "%s", ConfigGetGamePath() );
...
void DIUpdateRegisters( void )
{
	...
	sync_before_read( (void*)DI_BASE, 0x30 );
			...
			switch( read32(DI_SCMD_0) >> 24 )
			{
				...
				//	dbgprintf( "DIP:DVDRead%02X( 0x%08x, 0x%08x, 0x%08x )\n", read32(DI_SCMD_0) >> 24, Offset, Length, Buffer|0x80000000 );
					...
					if( FSTMode )
					{
						...
						FSTRead( GamePath, (char*)Buffer, Length, Offset );
						...
						s32 ret = f_read( &GameFile, (void*)Buffer, Length, &read );
				//		dbgprintf( "%d\n", read );
